...
export class AttributesEmitter {
  ...
  public emit(att: AttributeModel) {
    ...
    switch (true) {
      case (att.isProvider && att.isOptional): return this.emitProviderOptional(att);
      case (att.isProvider): return this.emitProviderRequired(att);
      case (att.computed && att.optional && !att.isRequired && att.isConfigIgnored): return this.emitOptionalComputedIgnored(att);
      case (att.computed && att.isOptional): return this.emitOptional(att);
  ...
  public emitInitializer(att: AttributeModel) {
    if (att.isProvider) {
      ...
      this.code.line(`this.${att.storageName} = new ${att.type.attributeName}(this, '${att.terraformName}', ${CUSTOM_DEFAULTS[att.terraformFullName]})`);
    }
    else if (att.isRequired) {
      this.code.line(`this.${this.getPutName(att)}(config.${att.name});`);
    }
    else {
      this.code.line(`this.${this.getPutName(att)}(config.${att.name} ?? new ${att.type.attributeName}(this, '${att.terraformName}', ${CUSTOM_DEFAULTS[att.terraformFullName]}));`);
    }
  }

  public emitAttributeAccessor(att: AttributeModel, isReadOnly: boolean) {
    const readOnly = isReadOnly || att.computed && !att.isOptional && !att.isRequired;
    const readOnlyStruct = att.type.struct?.isReadOnly ?? false;

    this.code.line();
    this.code.line(`// ${att.terraformName} - computed: ${att.computed}, optional: ${att.isOptional}, required: ${att.isRequired}`);
    this.code.openBlock(`public get ${att.name}()`);
      if (readOnlyStruct) {
        this.code.line(`return new ${att.type.attributeName}(this, '${att.terraformName}');`);
      }
      else {
        this.code.line(`return ${att.type.attributeName}.construct(this, '${att.terraformName}', ${readOnly ? 'undefined' : `this.internalValue?.${att.name}`});`);
      }
    this.code.closeBlock();
  }
  ...
  public determineMapType(type: AttributeTypeModel): string {
    ...
    if (process.env.DEBUG) {
      console.error(`The attribute ${JSON.stringify(type)} isn't implemented yet`)
    }
    return `any`
  }
  ...
  public getTypeToTerraform(type: AttributeTypeModel, varReference: string) {
    switch (true) {
      case (type.isList && type.isMap):
        return `cdktf.listMapper(cdktf.hashMapper(cdktf.${this.determineMapType(type)}ToTerraform))(${varReference})`;
      case (type.isStringList || type.isNumberList || type.isBooleanList):
        return `cdktf.listMapper(cdktf.${downcaseFirst(type.innerType)}ToTerraform)(${varReference})`;
      case (type.isList):
        return `cdktf.listMapper(${downcaseFirst(type.innerType)}ToTerraform)(${varReference})`;
      case (type.isMap):
        return `cdktf.hashMapper(cdktf.${this.determineMapType(type)}ToTerraform)(${varReference})`;
      case (type.isStringSet || type.isNumberSet || type.isBooleanSet):
        return `cdktf.setMapper(cdktf.${downcaseFirst(type.innerType)}ToTerraform)(${varReference})`;
      case (type.isSet):
        return `cdktf.setMapper(${downcaseFirst(type.innerType)}ToTerraform)(${varReference})`;
      case (type.isString):
