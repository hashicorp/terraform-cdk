...
export async function startServer(): Promise<number> {
  ...
  subprocess.stderr?.on("data", (chunk) => serverLogger.error(chunk.toString()));
  subprocess.stdout?.on("data", (chunk) => serverLogger.debug(chunk.toString()));

  // wait for server to be running
  const waitForServer = async (retries = 10, waitTime = 500): Promise<boolean> => {
    try {
  ...
  logger.debug(running ? 'Server is up' : 'Server did not manage to start up within alloted time');
