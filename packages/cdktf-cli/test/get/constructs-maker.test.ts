...
describe("constructsMaker", () => {
    describe("determineGoModuleName", () => {
        ...
        it("throws if go.mod is invalid", async () => {
            await expect(determineGoModuleName(invalidSubdir))
                .rejects
                .toThrow(`Could not determine the root Go module name. Found ${path.join(invalidSubdir, 'go.mod')} but failed to regex match the module name directive`);
        });

        it("works from subdirectory that does not exist yet", async () => {
            const moduleName = await determineGoModuleName(path.join(tmpDir, 'subdir', 'that', 'does', 'not', 'exist', 'yet'));
        ...
        it("throws if nothing could be found", async () => {
            ...
            await expect(determineGoModuleName(dir))
                .rejects
                .toThrow(`Could not determine the root Go module name. No go.mod found in ${dir} and any parent directories`);
        });
    });
});
