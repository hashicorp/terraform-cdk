  ...
  describe("diff", () => {
    ...
    it("fails if no stack specified", () => {
      ...
      return expect(cdktfProject.diff()).rejects.toMatchInlineSnapshot(
        `[Error: terraform init errored with: Error: Usage Error: Found more than one stack, please specify a target stack. Run cdktf diff <stack> with one of these stacks: first, second ]`
      );
    });
  });
