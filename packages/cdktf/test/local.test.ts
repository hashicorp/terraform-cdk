...
test('function local', () => {
    ...
    new TerraformLocal(stack, 'instance_ids', '${concat(aws_instance.blue.*.id, aws_instance.green.*.id)}')
...
test('local reference', () => {
    ...
    const resourceName = new TerraformLocal(stack, 'resource_name', 'my_resource');
