...
export class Go {
  ...
  importObject = {
    go: {
      ...
      "runtime.wasmWrite": (sp: number) => {
        ...
        fs.writeSync(
          fd,
          new Uint8Array(asMemory(this._inst.exports.mem).buffer, p, n)
        );
      },
      ...
      "syscall/js.valueGet": (sp: number) => {
        const result = Reflect.get(
          this._loadValue(sp + 8) as object,
          this._loadString(sp + 16)
        );
      ...
      "syscall/js.valueInstanceOf": (sp: number) => {
        this.mem.setUint8(
          sp + 24,
          this._loadValue(sp + 8) instanceof (this._loadValue(sp + 16) as any)
            ? 1
            : 0
        );
      },
  ...
  _loadString = (addr: number) => {
    ...
    return decoder.decode(
      new DataView(asMemory(this._inst.exports.mem).buffer, saddr, len)
    );
  };
}
