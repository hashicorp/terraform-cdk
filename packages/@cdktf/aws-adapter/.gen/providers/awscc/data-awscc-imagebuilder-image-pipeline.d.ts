import { Construct } from 'constructs';
import * as cdktf from 'cdktf';
export interface DataAwsccImagebuilderImagePipelineConfig extends cdktf.TerraformMetaArguments {
    /**
    * Uniquely identifies the resource.
    *
    * Docs at Terraform Registry: {@link https://www.terraform.io/docs/providers/awscc/d/imagebuilder_image_pipeline.html#id DataAwsccImagebuilderImagePipeline#id}
    */
    readonly id: string;
}
export declare class DataAwsccImagebuilderImagePipelineImageTestsConfiguration extends cdktf.ComplexComputedList {
    get imageTestsEnabled(): any;
    get timeoutMinutes(): number;
}
export declare function dataAwsccImagebuilderImagePipelineImageTestsConfigurationToTerraform(struct?: DataAwsccImagebuilderImagePipelineImageTestsConfiguration): any;
export declare class DataAwsccImagebuilderImagePipelineSchedule extends cdktf.ComplexComputedList {
    get pipelineExecutionStartCondition(): string;
    get scheduleExpression(): string;
}
export declare function dataAwsccImagebuilderImagePipelineScheduleToTerraform(struct?: DataAwsccImagebuilderImagePipelineSchedule): any;
/**
* Represents a {@link https://www.terraform.io/docs/providers/awscc/d/imagebuilder_image_pipeline.html awscc_imagebuilder_image_pipeline}
*/
export declare class DataAwsccImagebuilderImagePipeline extends cdktf.TerraformDataSource {
    static readonly tfResourceType: string;
    /**
    * Create a new {@link https://www.terraform.io/docs/providers/awscc/d/imagebuilder_image_pipeline.html awscc_imagebuilder_image_pipeline} Data Source
    *
    * @param scope The scope in which to define this construct
    * @param id The scoped construct ID. Must be unique amongst siblings in the same scope
    * @param options DataAwsccImagebuilderImagePipelineConfig
    */
    constructor(scope: Construct, id: string, config: DataAwsccImagebuilderImagePipelineConfig);
    get arn(): string;
    get containerRecipeArn(): string;
    get description(): string;
    get distributionConfigurationArn(): string;
    get enhancedImageMetadataEnabled(): cdktf.IResolvable;
    private _id;
    get id(): string;
    set id(value: string);
    get idInput(): string;
    get imageRecipeArn(): string;
    get imageTestsConfiguration(): any;
    get infrastructureConfigurationArn(): string;
    get name(): string;
    get schedule(): any;
    get status(): string;
    tags(key: string): string;
    protected synthesizeAttributes(): {
        [name: string]: any;
    };
}
