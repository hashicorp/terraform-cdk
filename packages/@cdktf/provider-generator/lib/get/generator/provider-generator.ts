...
export class TerraformProviderGenerator {
  ...
  private emitProvider(fqpn: string, provider: Provider) {
    ...
    // if exports is set, export those names, else export "*"
    const files: { file: string; exports?: string[] }[] = [];
    resourceModels.forEach((resourceModel) => {
      if (resourceModel.namespace) {
        ...
        files.push({ file: this.emitResourceFile(resourceModel) });
      }
    });

    for (const [namespace, resources] of Object.entries(namespacedResources)) {
      files.push({
        file: this.emitNamespacedResourceFile(namespace, resources),
        exports: [namespace],
      });
    }
    ...
    if (provider.provider) {
      ...
      files.push({ file: this.emitResourceFile(providerResource) });
    }
  ...
  private emitIndexFile(
    provider: string,
    files: { file: string; exports?: string[] }[]
  ): void {
    ...
    for (const file of files) {
      this.code.line(
        `export ${
          file.exports ? `{ ${file.exports.join(", ")} }` : "*"
        } from './${file.file.replace(`${folder}/`, "").replace(".ts", "")}';`
      );
    }
    this.code.line();
