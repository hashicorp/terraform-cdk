...
export class ConstructsMaker {
  ...
  private async generateTarget(target: ConstructsMakerTarget) {
    const endGenerateTimer = logTimespan("Generate TS");
    await this.generateTypescript(target);
    endGenerateTimer();
  ...
  // TODO: make parallelism optional / configurable
  // TODO: Don'wait for all TS generations to be done before starting to generate the other language
  public async generate() {
    const endGenerateAllTimer = logTimespan("Generating code");

    await Promise.all(
      this.targets.map((target) => this.generateTarget(target))
    );
