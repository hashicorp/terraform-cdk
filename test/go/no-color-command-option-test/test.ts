...
describe("no-color option for cdktf deploy, diff, destroy", () => {
  ...
  it("contains color formatting in cdktf deploy", async () => {
    const result = await execa("cdktf", ["deploy", "--auto-approve"], {
      env: driver.env,
      cwd: driver.workingDirectory,
    });
    expect(result.stdout).toMatch(ansiRegex);
  });
  ...
  it("contains color formatting in cdktf diff", async () => {
    const result = await execa("cdktf", ["diff"], {
      env: driver.env,
      cwd: driver.workingDirectory,
    });
    expect(result.stdout).toMatch(ansiRegex);
  ...
  it("contains color formatting in cdktf destroy", async () => {
    const result = await execa("cdktf", ["destroy", "--auto-approve"], {
      env: driver.env,
      cwd: driver.workingDirectory,
    });
    expect(result.stdout).toMatch(ansiRegex);
