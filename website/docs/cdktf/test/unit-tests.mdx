...
- **TypeScript (with Jest):** Follow Jest's [Getting Started guide](https://jestjs.io/docs/getting-started) for testing with TypeScript. Then, add `cdktf/types/jest` to the [`types` array](https://www.typescriptlang.org/tsconfig#types) in your `tsconfig.json` file and the following to a [setup file](https://jestjs.io/docs/configuration#setupfiles-array):
...
- **TypeScript (with Vitest):** Follow Vitest's [Getting Started guide](https://vitest.dev/guide/). Then, add `cdktf/types/vitest` to the [`types` array](https://www.typescriptlang.org/tsconfig#types) in your `tsconfig.json` file and the following to a [setup file](https://vitest.dev/config/#setupfiles):

  ```js
  const cdktf = require("cdktf");
  const { expect } = require("vitest");
  cdktf.Testing.setupVitest(expect);
  ```
...
Snapshot tests are useful when you want to make sure your infrastructure does not change unexpectedly. Snapshot Testing is only supported in Typescript with [Jest](https://jestjs.io/docs/snapshot-testing) or [Vitest](https://vitest.dev/guide/snapshot.html).
