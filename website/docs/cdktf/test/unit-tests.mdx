...
Testing your application can give you faster feedback cycles and guard you against unwanted changes. Testing is supported in Typescript with Jest. For all other languages, you can use any existing testing framework that supports assertions. 

We generate all files necessary to start testing when you run `cdktf init` so that you can start writing tests right away.

If you want to add jest to an existing Typescript project, please [follow their guide](https://jestjs.io/docs/getting-started). Then, you need to add these lines in a [setup file](https://jestjs.io/docs/configuration#setupfiles-array):
...
If you want to add pytest to an existing Python project, please [follow their guide](https://docs.pytest.org/en/7.1.x/getting-started.html). The CDKTF specific assertions can be found in the cdktf package under `Testing`.

If you want to add JUnit to an existing Java project, please [follow their guide](https://maven.apache.org/plugins-archives/maven-surefire-plugin-2.12.4/examples/junit.html). The CDKTF specific assertions can be found in the cdktf package under `Testing`.

If you want to add Xunit to an existing C# project, please [follow their guide](https://xunit.net/docs/getting-started/netcore/cmdline). The CDKTF specific assertions can be found in the cdktf package under `Testing`.

If you want to add testing to an existing Go project, please [follow their guide](https://go.dev/doc/tutorial/add-a-test). The CDKTF specific assertions can be found in the cdktf package under `Testing`.
...
The examples for other languages use `Testing.synth` to test a part of the application. CDKTF returns a JSON string representing the synthesized HCL-JSON is returned. Then, the custom assertions under `Testing` in the `cdktf` package verify the code acts as intended.
...
Snapshot tests are useful when you want to make sure your infrastructure does not change unexpectedly. Snapshot Testing is currently only supported in Typescript with jest You can read more about them in the [Jest docs](https://jestjs.io/docs/snapshot-testing).
