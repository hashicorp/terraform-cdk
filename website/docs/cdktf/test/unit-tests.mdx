...
For Typescript we generate all files necessary to run jest when you run `cdktf init` so that you can start writing tests right away. If you want to add jest to an existing Typescript project, please [follow their guide](https://jestjs.io/docs/getting-started). Then, you need to add these lines in a [setup file](https://jestjs.io/docs/configuration#setupfiles-array):
...
For all other supported languages the CDKTF specific assertions can be found in the cdktf package under `Testing`.
...
from app import MyApplicationsAbstraction # Could be a class extending from cdktf.Resource
...
import com.hashicorp.cdktf.MyApplicationsAbstraction; // Could be a class extending from cdktf.Resource
...
```csharp
using Xunit;
using HashiCorp.Cdktf; // MyApplicationsAbstraction - Could be a class extending from cdktf.Resource
...
class TestApplication:
  ...
  stack = new TerraformStack(Testing.app(), "test")
