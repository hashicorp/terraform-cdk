...
Your application needs somewhere to store [Terraform state](/language/state). Add the `--local` flag to created a scaffolded project that is pre-configured to use a [local backend](/language/settings/backends/enhanced/local). This means all terraform operations will happen on your local machine.
