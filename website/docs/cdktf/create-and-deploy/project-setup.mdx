...
~> **Important**: The synthesized Terraform config might contain credentials or other sensitive data that was provided as input for the `cdktf` application. Use [`TerraformVariable`s](/cdktf/concepts/variables-and-outputs) to pass secrets configured as [workspace-specific variables](cloud-docs/workspaces/variables/managing-variables#workspace-specific-variables) in TerraformCloud.
