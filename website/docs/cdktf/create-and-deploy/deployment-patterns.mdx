...
You can configure your CI/CD environment to automatically deploy your CDKTF applications. This page explains different types of deployment patterns to help you choose the methods that are most appropriate for your use case.
...
Terraform Cloud lets you review the current state of your infrastructure and run progress in the Terraform Cloud UI. You can also use Terraform Cloud as the source of secrets via [workspace variables](/cloud-docs/workspaces/variables). If you use the local execution mode or any other backend you must specify the credentials for your providers locally through environment variables.

To use Terraform Cloud with a [CLI-driven Run Workflow](/cloud-docs/run/cli), you must enable [remote execution](/cloud-docs/run#remote-operations) in your Terraform Cloud workspace.
...
No matter if you manage your secrets through Terraform Cloud (in `remote-execution` mode) or through your normal CI, secrets should never appear in your synthesized code. [This is why you should read Secrets with Terraform Variables](/cdktf/create-and-deploy/best-practices#read-secrets-with-terraform-variables).
...
If you are using GitHub Actions and you want a default review / deployment workflow, you can use the [Terraform CDK GitHub Action](https://github.com/marketplace/actions/terraform-cdk-action). It provides you with comments on your PRs and a default review workflow. If you need more customization, please take a look at the next example.
...
If you have a general purpose CI like GitHub Actions or CircleCI configured, use the `cdktf deploy --auto-approve <your stacks>` command to trigger a deployment. This deploys your stacks in the right order and in parallel. To do set up this workflow, you must install the following software:
...
If you want to share another solution on this page, please [open a GitHub issue](https://github.com/hashicorp/terraform-cdk/issues/new?assignees=&labels=enhancement%2C+new&template=feature-request.md&title=) or submit a pull request for this page.
