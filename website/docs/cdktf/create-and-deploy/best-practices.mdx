...
`TerraformVariable`s should only be used at the stack-level, nesting them in custom constructs makes the interface unclear (some information might be passed into the constructor of the construct and some is gathered through `TerraformVariable`s), it hides which `TerraformVariable`s need to be configured for the CDKTF program to run, and nesting something in a custom construct changes the logical id (which can be overwritten by `.overwriteLogicalId("...")`).
