...
```
cdktf synth [OPTIONS] <stacks..>

Synthesizes Terraform code for the given app in a directory.

Positionals:
  stacks  Stacks to output when using --json flag
...
Synthesize code for an application and output the Terraform JSON configuration for two stacks.

```bash
$ cdktf synth --json stackA stackB
```
