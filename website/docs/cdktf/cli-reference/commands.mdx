...
For more infos in the `--outputs-file` option, [see the `output` command below](/cdktf/cli-reference/commands#output).
...
```json
{
  "MyStack": {
    "MyConstructInstance": {
      "MyOutput": "my-value",
      "MySecretOutput": "my-secret-value" // <sensitive> if --outputs-file-include-sensitive-outputs is not set
    }
  }
}
```
