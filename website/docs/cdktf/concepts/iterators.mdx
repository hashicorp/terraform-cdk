...
Please note that it is not possible to access the index of items when iterating over lists. This is because Terraform requires sets for iteration and CDKTF implicitly converts lists to sets when iterating over them. This behaviour is on purpose and helps avoiding accidental deletion and recreation of resources when their indices change. If you need an index, you can use `count` and an [escape hatch](/cdktf/concepts/providers-and-resources#escape-hatch) using the [`count.index` property](/language/meta-arguments/count).
