...
For module inputs that use the `map` type, like `map(string)` or `list(map(string))`, you must specify the map values as strings. You must also ensure that these strings follow the required format listed in the module's documentation. For example, the module may specify that the keys must be in snake case. 
