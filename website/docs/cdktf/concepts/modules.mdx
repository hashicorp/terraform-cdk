...
In general modules can be configured like a resource, but there is a special case:

When you have a module input that uses the `map` type, e.g. `map(string)` or `list(map(string))` you need to specify the values in the map as strings. The keys within the map must be as Terraform expects them, you need to follow the documentation of the HCL Module, so if it specifies snake cased keys they have to be snake cased in the module configuration as well.
