...
The CLI will error if you try to deploy something without deploying the dependencies first and if you try to destroy something without destroying the dependant stacks first. If you don't want these safe-guards you can add the `--ignore-missing-stack-dependencies` to deploy / destroy command.
