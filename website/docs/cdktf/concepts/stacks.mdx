...
You can deploy and destroy multiple stacks at once by specifying multiple stacks in the `cdktf deploy` and `cdktf destroy` command or by using a wild card glob `cdktf deploy '*-production'`.

Visit our [best practices](/cdktf/create-and-deploy/best-practices#separate-business-units-with-stacks) to find out how stacks should be strucutred.
