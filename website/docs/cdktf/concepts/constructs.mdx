...
You can improve the feedback cycles about security concerns for developers from tools later in the workflow like code review or CI solutions like Sentinel to the point where the configuration is authored. This can be done through type hints (disallowing options that are not safe on the type system level) or through validation. Validations allow dynamic verification and are shown to users as hints during `cdktf synth`, if they are on the error level they also fail the synth.
