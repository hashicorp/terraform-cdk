...
Since constructs can also represent a collection of resource, they are often compared with Terraform modules, but constructs provide a superset of what Terraform modules can do.

## Various use cases of constructs

- Terraform modules are fixed in the sense that they can generate resources based on hcl configuration, but constructs are not constrained to just creating, and can also modify, enrich, and validate resources created outside of the construct.

- Constructs can can build complex resources that can be used as any other provider supplied resource without using a different syntax like in Terraform.

- Constructs allow strict type checking and parameter validations for creating resources. This is not possible in Terraform modules.
...
// TODO: this example does not yet work, but it's possible to do
class OutputAllBucketNames extends Construct {
  constructor(scope: Construct, name: string) {
    ...
    new TerraformOutput(this, "all_bucket", {
      value: new Lazy({
        produce: () => {
          return buckets;
        },
...
Since constructs are classes, they allow methods to programmatically add or modify infrastructure based on conditions. This enables the construct's consumer to selectively enhance their infrastructure.
...
Constructs can also be used to represent relationships between different parts of the infrastructure in a type-safe manner. Creating relationships through constructs also has the benefit that the relationship is also easily understandable in most modern IDEs.

In the following example, we have a contrived `Cluster` construct that allows various webservices to be registered with it. During development, any consumer of the Cluster class will be able to see that any `Cluster` construct accepts `Webservice` constructs to be connected to it.
...
If you have complex architectural patterns you can codify them and have an abstraction where you can create a flexible architecture with less work. In this DataPipeline example you can see that each of the steps in the pipeline are bringing their own logic on how to connect to the next step and how to write their output into an S3Bucket. This means as long as a set of resources can fulfil this contract one could build a pipeline using e.g. LambdaFunctions, Ec2 Instances, and CloudwatchEvents if one implemented the `PipelineStep` class for these types.
...
You can improve the feedback cycles about security concerns for developers from tools later in the workflow like code review or CI solutions like sentinel to the point where the cofiguration is authored. This can be done through type hints (disallowing options that are not save on the type system level) or through validation. Validations allow dynamic verifications and are shown to users as hints during `cdktf synth`, if they are on the error level they also fail the synth.
...
describe("Construct is of correct compostion and plans successfully", () => {
...
Make custom matchers for you own use case

[TODO]: find interesting use case, see if our current matchers are extendable
...
Advantages over Terraform Module Testing

- Testing done natively in your chosen CDKTF configuration language
- No need to create objects in real remote state that are subsequently destroyed
-
