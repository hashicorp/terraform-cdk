...
~> **Important**: If you directly pass values to the constructs the synthesized Terraform config will contain these values which includes credentials or other sensitive data that was provided as input for the `cdktf` application. If you plan to commit the generated `cdk.tf.json` to version control, use input variables for secrets instead.
