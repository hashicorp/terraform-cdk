...
description: |-
  Write highly configurable construct classes that provide configuration guidance for a wide range of use cases.
---
...
When you write constructs in the same language as your CDKTF application, you can use the same importing mechanisms you use with other applications. Refer to [Constructs](/link) in the CDKTF Concepts documentation for more details about using constructs.
...
The second option is using Terraform Modules as an intermediate layer. You can use the [cdktf-tf-module-stack](https://github.com/cdktf/cdktf-tf-module-stack) to transform your code into a Terraform module and then import the module into any CDKTF application and let CDKTF [generate the local code bindings](/cdktf/concepts/modules#add-module-to-cdktf-json) required to use it. This approach lets you write your construct in any language and requires less setup than using `jsii`. However, you are limited to the structure and functionality available in a Terraform module. For example, you cannot use programming functionality like enums or methods to provide advanced configuration options.
...
Let users overwrite your construct's attributes when possible. This prevents users from having to overwrite or modify your construct’s outputs if they need to change your defaults.
...
The following example shows how template methods can be used to overwrite the mechanism through which secrets are retrieved for a docker container depending on the class used.
...
The following example shows how one can use a static function to provide an alternative entry-point to the usage of the construct.
...
You should also include enums to help users understand which configuration options are available for each attribute. Using methods to build functionality can also provide additional guidance. For example, creating a method called `addAIComputingNode` helps the user understand what the code is doing and how they might customize it much more than a configuration where they must configure one key in an array for AI workloads.
